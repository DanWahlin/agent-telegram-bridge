...
Linux uses `/proc` to prove the ACP child working directory and expose retained attachment descriptors. macOS uses fixed-argument `/usr/sbin/lsof` and `/bin/ps` process inspection, plus device/inode and digest revalidation for attachments. When an agent lacks embedded-context support, the macOS `resource_link` fallback exposes a validated filesystem path; another process running as the same user could replace that path after handoff. Native Windows is not supported; use WSL2 so the Linux security adapter remains available.
